...
//
// When implementing get(), throw FileNotFoundError from ./index on a NoSuchKey
// response rather than letting the SDK's error escape — the download route
// distinguishes 404 from 500 on exactly that.
export class S3Storage implements Storage {
  ...
  async get(_path: string): Promise<Readable> {
    throw new Error("S3Storage.get not implemented — wire up @aws-sdk/client-s3 before enabling.");
  }

 ...
 */
function routeNameForHref(href: string): string {
  const path = href.slice(1);
  return path === "students" ? "students/index" : path;
}

/**
 * The full universe of route files under this group, derived from every
 * role's nav data rather than hand-maintained — a role gaining a new tab
 * or sidebar entry (with a route file added to match) needs no edit here.
 ...
 * has it there.
 */
const ALL_ROUTE_NAMES = Array.from(
  new Set(
    Object.values(navByRole)
      .flatMap((nav) => [...nav.tabs, ...nav.sidebar])
      .map((item) => routeNameForHref(item.href)),
  ),
);
